    ...
    public class CartApiController : ControllerBase
    {
        ...
        public async Task<IActionResult> AddToCart([FromBody] AddCartItemRequest request)
        {
            ...
            if (!string.IsNullOrWhiteSpace(size))
            {
                ...
                if (sizes.Count > 0 && !sizes.Contains(size, StringComparer.OrdinalIgnoreCase))
                {
                    return BadRequest(new { success = false, message = "Size không hợp lệ cho sản phẩm này" });
                }
            }
            ...
            var currentQtyForLine = existing?.Quantity ?? 0;
            if (currentQtyForLine + qty > product.Stock)
            {
        ...
        public async Task<IActionResult> UpdateQuantity(int productId, [FromBody] UpdateCartQuantityRequest request)
        {
            ...
            {
                if (item.Product != null && request.Quantity > item.Product.Stock)
                {
                    return BadRequest(new { success = false, message = "Số lượng vượt quá tồn kho" });
                }
