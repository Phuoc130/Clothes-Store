...
public class DashboardController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        if (dashboardResponse.IsSuccessStatusCode)
        {
            ...
            if (dashboard != null)
            {
                ...
                model.RevenueSeries = dashboard.RevenueSeries;
            }
        }
    ...
    {
        var client = _httpClientFactory.CreateClient("BackendApi");
        using var response = await client.PostAsync($"api/orders/admin/{orderId}/confirm-transfer", null);

        if (response.IsSuccessStatusCode)
        {
            TempData["AdminNotice"] = $"Da xac nhan thanh toan don #{orderId}";
        }
        else
        {
            var body = await response.Content.ReadAsStringAsync();
            TempData["AdminNotice"] = $"Khong the xac nhan don #{orderId}: {body}";
        }
